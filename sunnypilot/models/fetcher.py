...
class ModelFetcher:
  ...
  def _get_local_bundles(self) -> list[custom.ModelManagerSP.ModelBundle]:
    ...
    try:
      ...
      cloudlog.warning(f"Failed to parse local models catalog {local_path}: {e}")
      return []
  ...
  def get_available_bundles(self) -> list[custom.ModelManagerSP.ModelBundle]:
    ...
    if cached_data and not is_expired:
      ...
      return self.model_parser.parse_models(cached_data) + self._get_local_bundles()

    fetched_bundles = self._fetch_and_cache_models()
    if fetched_bundles is not None:
      return fetched_bundles + self._get_local_bundles()
